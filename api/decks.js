...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    return res.status(200).json({ ok: true, decks: buildDeckCatalog(index) });
  }
  ...
  const adminKey = req.headers['x-deck-admin-key'];
  if (!process.env.DECK_ADMIN_KEY || adminKey !== process.env.DECK_ADMIN_KEY) {
    return res.status(401).json({ error: 'Unauthorized publish key' });
  }
  ...
  if (!manifest) return res.status(400).json({ error: 'Missing deck manifest payload' });
  ...
  try {
    ...
    return res.status(200).json({
      ...
      mode,
      published: entry,
      decks: buildDeckCatalog(nextIndex)
    });
  } catch (error) {
